...
export default class Scheduler extends Component {
  ...
  validateFile(file) {
    let allow_types = ['image/jpeg', 'image/png'];
    if (!allow_types.includes(file.type)) {
      toastr.error(
        file.type +
          ' mime type not supported. We only accept JPEG and PNG images',
        'Invalid file type'
      );
      return false;
    }
  ...
  onPageSelect(e) {
    if (e.target.value.length) {
      this.setState({
        page: this.state.pages.filter(page => page.id === e.target.value)[0]
      });
    } else {
      this.setState({
        page: { id: '', name: '' }
